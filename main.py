...
def gen(ext, obs, shy, calm, scared, intelligent, vigilant, persistent, affectionate, friendly, solitary, rough, dominant, aggressive, impulsive, predictable, distracted, abundance, predBirds, predMamm):
    TEMPLATES = [
...
def getLevelAtribute(rank):
    if(rank == 0):
        ...
        return random.choice(["shows little", "displays some", "has demonstrated a modest amount of", "is somewhat showing"])
    elif(rank == 3):
        return random.choice(["shows moderate", "displays a fair amount of", "has shown moderate", "demonstrates moderate"])
    elif(rank == 4):
        return random.choice(["is strongly showing", "displays a strong", "has significantly shown", "demonstrates a high level of"])
    elif(rank == 5):
        return random.choice(["is extremely showing", "displays an intense", "has shown a very high level of", "demonstrates maximum"])
    else:
        return ""
...
def generateDescription(rankings, breed):
    if("No breed" in breed or "Other" in breed or "Unknown breed" in breed):
       ...
       print( gen(getLevelAtribute(rankings[0]), getLevelAtribute(rankings[1]), getLevelAtribute(rankings[2]), getLevelAtribute(rankings[3]), getLevelAtribute(rankings[4]), getLevelAtribute(rankings[5]),getLevelAtribute(rankings[6]),getLevelAtribute(rankings[7]), getLevelAtribute(rankings[8]),getLevelAtribute(rankings[9]), getLevelAtribute(rankings[10]),getLevelAtribute(rankings[11]), getLevelAtribute(rankings[12]), getLevelAtribute(rankings[13]),getLevelAtribute(rankings[14]),getLevelAtribute(rankings[15]),getLevelAtribute(rankings[16]),getLevelAtribute(rankings[17]),getLevelAtribute(rankings[18]),getLevelAtribute(rankings[19])))
